...
xmlhttp.open("POST","updateppd.php",true);
...
<title>Display pic Context-Mypixal</title>
...
try {
	...
	session_start();
	if(!isset($_SESSION['uploadp']))//if session is not set.
	{
	...
	<div class="container">
		<div class="jumbotron jumbotron-custom1">
			<center>
			<p class="heading">Display-pic Contest</p>
			<form  class="navbar-form" enctype="multipart/form-data" action="fetchppd.php" method="post">
			...
			<?php
			echo "</center>";
			$query=mysqli_query_or_throw_error($con,"select `id` from `picweekpp`");
			$no=mysqli_num_rows($query);
			if($no!=0)
			{
			$query=mysqli_query_or_throw_error($con,"select * from `picweekpp` order by `likes` desc limit 0,5");
			?>
			<center>
			...
			<div class="row">
				<?php
					while($row=mysqli_fetch_assoc($query))
					{
							...
							echo "<script type='text/javascript'>alert('Congratulations!.You Have Earned 3 Points Because One of Your uploaded Pic Get On To The Top 5 of This Category');</script>";//alert.
							$q=mysqli_query_or_throw_error($con,"update `picweekpp` set `status`='1' where `id`='$id'");//update status.
							}
							...
							if((mysqli_num_rows($q))!=0)//if yes.
							{
								...
								echo "<script type='text/javascript'>alert('Congratulations!.You Have Earned 2 Points Because One of Your Liked Pic Get On To The Top 5 of This Category');</script>";//alert.
	...
	<?php
	$flag1=1;
	$query=mysqli_query_or_throw_error($con,"select `id` from `picweekpp` order by `id` asc limit 0,1");
	$firstid=mysqli_fetch_assoc($query);
	$firstid=$firstid["id"];
	$query=mysqli_query_or_throw_error($con,"select * from `picweekpp` order by `id` desc limit 0,54");//select recent 54 images from 'picweekcp'.
	...
	<div class="container">
		<div class="jumbotron jumbotron-custom1">
			<center>
				<p class="heading">Display pics of this week</p>
			<div class="row">
			<?php
			...
			{
				$lastid--;
				echo "<center><a class='btn btn-primary see-all' href='profilemored.php?lastid=$lastid'>More</a></center>";//see all link.
			}
			?>
			</div>
		</div>
