...
xmlhttp.open("POST","updatecpd.php",true);
...
try {
	session_start();
	if(!isset($_SESSION['uploadc']))//if session is not set.
	{
	...
	<div class="container">
		<div class="jumbotron jumbotron-custom1">
			<center>
			<p class="heading">Selfie Contest</p>
			<form  class="navbar-form" enctype="multipart/form-data" action="fetchcpd.php" method="post">
			...
			<?php
			echo "</center>";
			$query=mysqli_query_or_throw_error($con, "select `id` from `picweekcp`");
			$no=mysqli_num_rows($query);
			if($no!=0)
			{
			$query=mysqli_query_or_throw_error($con, "select * from `picweekcp` order by `likes` desc limit 0,5");
			?>
			<center>
			...
			<div class="row">
				<?php
					while($row=mysqli_fetch_assoc($query))
					{
							...
							$likes=$row['likes'];
							$status = $row['status'];
							if($profile_id == $profile_id_real && $status == 0)//if yes.
							{
							...
							$q=mysqli_query_or_throw_error($con, "update `picweekcp` set `status`='1' where `id`='$id'");//update status.
							}
	...
	<?php
	$flag1=1;
	$query=mysqli_query_or_throw_error($con, "select `id` from `picweekcp` order by `id` asc limit 0,1");
	$firstid=mysqli_fetch_assoc($query);
	$firstid = $firstid["id"];
	$query=mysqli_query_or_throw_error($con, "select * from `picweekcp` order by `id` desc limit 0,54");//select recent 54 images from 'picweekcp'.
	if((mysqli_num_rows($query))!=0)
	{
		...
		<div class="container">
			<div class="jumbotron jumbotron-custom1">
				<center>
					<p class="heading">Selfies of this week</p>
				<div class="row">
				<?php
				...
				<?php
						echo "<div class='thumbnail'><a href='getalbumfull.php?name=$name'><img src='album_th/$name' width='90%' /></a></br>";
				...
				{
					$lastid--;
					echo "<center><a class='btn btn-primary see-all' href='selfiemored.php?lastid=$lastid'>More</a></center>";//see all link.
				}
				?>
				</div>
			</div>
